...
//
// This Version - 2.0.1.8 Monday 05-SEP-2016
...
//
// 25/07/2021 - Added 2 new methods: Install ODBC32 and InstallODBC64:
//              to install ODBC driver into the registry - test.

interface

uses Windows, Classes, Graphics, Forms, Controls, StdCtrls, Buttons,
  ExtCtrls, Grids, DBGrids, DB, ADODB, Provider, DBClient,
  SqlExpr, strUtils, sysUtils, inifiles, DateUtils, Variants, Registry;
...
type TstrArray = Array of String;
  ...
  public
    ...
    function SelectDates(DBTable,ConnString,SearchDateField: string;
      SortField: string; ReverseDirection: boolean; DateFrom,DateTo: TDateTime;
      StripTime,ChangeQueryString:boolean): integer;overload;
    function SelectDates(DBTable,ConnString,SearchDateField: string;
      SortField: string; ReverseDirection: boolean; DateFrom,DateTo: TDateTime;
      Filter:string; StripTime,ChangeQueryString:boolean): integer;overload;
    ...
    function CreateSingleRecArray(AADOQuery: TADOQuery; Restart:boolean): TstrArray;
    function GetFieldPosition(TableName,FieldName: String; connString:widestring): integer;
    function SearchField(DBTable,ConnString,SearchField, SearchValue: String;
      ExactMatch,ChangeQueryString: boolean): integer;overload;
    function SearchField(DBTable,ConnString,ReturnField,
      SearchField,SearchValue: String; ExactMatch,ChangeQueryString: boolean): string;overload;
    function GetNewFieldValue(FieldName,FieldValue,TableName,ConnString:string):string;
    ...
    function InstallODBC32(ODBCType: string): integer;
    function InstallODBC64(ODBCType: string): integer;
    ...
    property GetExceptionMsg: string read FExceptionError write FExceptionError;
...
begin
  ...
  if Length(strTime)>0 then begin
    ...
    try
      TrystrToTime(strTime,dtTime);
    except begin
      ErrMsg:= 'Error: Time Not Valid';
      IsError:= True;
    end;
    end;
  end;
...
function TDBOperations.CreateSingleRecArray(AADOQuery: TADOQuery; Restart:boolean): TstrArray;
var
...
begin
  ...
  // as this function is used again and again within the same run of the
  // application !! ????
  // 31-07-2021 03:20 => Added extra boolean parameter - so calling
  // code can decide whether to reset array or not.
  /////////////////////////////////////////////////////////////////////////////
  QueryRows:= 1; //record found goes into single-dimensional array NOT 0 so 1st record not missed.
  FieldList:= self.ExtractFieldNames(AADOQuery);
  if Restart then SetLength(SingleRecArray,1) else
...
function TDBOperations.SearchField(DBTable,ConnString,SearchField,
  SearchValue: String; ExactMatch, ChangeQueryString: boolean): integer;
var
...
begin
  ...
  if (FieldType = 'ftInteger') or (FieldType = 'ftAutoInc') then begin
    ...
    if ExactMatch then
      SQLQuery:= 'SELECT * FROM '+DBTable+' WHERE '+SearchField+' = '+chr(39)+SearchValue+chr(39)
    else
...
function TDBOperations.SearchField(DBTable,ConnString,ReturnField,
  SearchField,SearchValue: String; ExactMatch,ChangeQueryString: boolean): string;
var
  NewYear,NewMonth,NewDay: word;
  NewHour,NewMin,NewSec,MSec: word;
  NewDate:string;
...
begin
  ...
  if (FieldType = 'ftInteger') or (FieldType = 'ftAutoInc') then begin
    ...
    //if any more searchfields then add:
    //SQLQuery:= SQLQuery+' AND Time('+SearchDateField+') = '+chr(39)+NewTime+chr(39);
  searchQuery:= TADOQuery.Create(nil);
  try
    searchQuery.ConnectionString:= ConnString;
    searchQuery.Close;
...
function TDBOperations.SelectDates(DBTable,ConnString,SearchDateField: string;
  SortField: string; ReverseDirection: boolean; DateFrom,DateTo: TDateTime;
  StripTime,ChangeQueryString:boolean): integer;
var
  NewYear,NewMonth,NewDay: word;
  NewHour,NewMin,NewSec,MSec: word;
...
function TDBOperations.SelectDates(DBTable,ConnString,SearchDateField: string;
  SortField: string; ReverseDirection: boolean; DateFrom,DateTo: TDateTime;
  Filter:string; StripTime,ChangeQueryString: Boolean): integer;
var
  NewYear,NewMonth,NewDay: word;
  NewHour,NewMin,NewSec,MSec: word;
  NewDateFrom,NewDateTo:string;
  ...
  if VarIsArray(V) then
    ...
    case VarType(V) of
      ...
      varEmpty:
        Result := '#EMPTY';
      varDate:
        Result := FormatDateTime('dd-mmm-yyyy hh:nn:ss', V)
    else
      Result := VarToStr(V);
    end;
  if IncludeType then
...
function TDBOperations.GetADOQuery: TADOQuery;
begin
  Result:= self.fADOQuery;
end;
...
function TDBOperations.InstallODBC32(ODBCType: string): integer;
var
  myResult: integer;
  fReg: TRegistry;
begin
  //Install ODBC driver into registry -
  //From D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win32\mysql-connector-odbc-noinstall-8.0.26-win32\lib
  //Added by Daniel Goss: 25/07/2021
  myResult:= 0;
  try
    ...
    if ODBCType = 'ODBC8' then
      fReg.WriteString('Driver', 'D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win32\mysql-connector-odbc-noinstall-8.0.26-win32\lib\myodbc8a.dll')
    else
      //fReg.WriteString('Driver', 'D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win32\mysql-connector-odbc-noinstall-8.0.26-win32\lib\myodbc5.dll');
...
function TDBOperations.InstallODBC64(ODBCType: string): integer;
var
  myResult: integer;
  fReg: TRegistry;
begin
  ...
  try
    ...
    fReg.WriteString('Description', ' ');
    if ODBCType = 'ODBC8' then
      //D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win64\mysql-connector-odbc-noinstall-8.0.26-winx64\lib
      fReg.WriteString('Driver', 'D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win64\mysql-connector-odbc-noinstall-8.0.26-win64\lib\myodbc8a.dll')
    else
      //fReg.WriteString('Driver', 'D:\DELPHI 2021\Delphi XE2 Projects\DelphiXE2_EnergyCalc2\Win32\mysql-connector-odbc-noinstall-8.0.26-win32\lib\myodbc5.dll');
